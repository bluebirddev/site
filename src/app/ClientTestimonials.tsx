...
export function ClientTestimonials() {
    ...
    return (
        <div className="w-full max-w-full flex flex-col items-center overflow-hidden pt-32 lg:pt-48 ">
            <div className="space-y-8 flex flex-col items-center pb-24">
                <h3 className="pill-heading">Client Testimonials</h3>
            ...
            <div className="w-full lg:w-[780px] relative">
                <div className="absolute inset-0 mx-auto flex items-center">
                    <button
                        className={
                            'absolute left-[0] lg:left-[-4px] z-10 h-14 w-14 rounded-full bg-white flex items-center justify-center' +
                            (index === 0 ? ' bg-opacity-20' : '')
                        }
                        disabled={index === 0}
                        onClick={() => setIndex(index - 1)}
                    >
                        <ChevronLeftIcon className="h-8 text-black" />
                    </button>
                    <button
                        className={
                            'absolute right-[0px] lg:right-[-4px] z-10 h-14 w-14 rounded-full bg-white flex items-center justify-center' +
                            (index === blocks.length - 1 ? ' bg-opacity-20' : '')
                        }
                        onClick={() => setIndex(index + 1)}
                        disabled={index === blocks.length - 1}
                    >
                        <ChevronRightIcon className="h-8 text-black" />
                    </button>
                </div>
                <div
